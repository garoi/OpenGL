...
float llum2 [4] = {1.0, 1.0, 1.0, 1.0};
float llum3[4] = {1.0, 0, 0, 1.0};
int PosEscena = 0;
...
void initPosCamera() {
    ...
    if (camNinot) {
        ca.up[1] = 1;
        gluLookAt(2.5, 2, 2.5, Mov.X , 2, Mov.Z , ca.up[0], ca.up[1], ca.up[2]);
    }
...
void initCamaraPers() {
    ...
    if (not primeraPersona and not camNinot) {
...
void llumModel() {
    glPushMatrix();
        float pos [4] = {Mov.X, 1.0, Mov.Z, 1.0};
        glLightfv(GL_LIGHT2, GL_DIFFUSE, llum2);
        glLightfv(GL_LIGHT2, GL_SPECULAR, llum2);
...
void llumEscena() {
  glPushMatrix();
      ...
      float posllum3[4] = {0, 2.5, 0, 1.0};
      glLightfv(GL_LIGHT3, GL_DIFFUSE, llum3);
      glLightfv(GL_LIGHT3, GL_SPECULAR, llum3);
      glLightfv(GL_LIGHT0, GL_POSITION, posllum3);
...
void teclat(unsigned char caracter, int x, int y) {
    if (caracter == 'h'){
        ...
        cout << "# - prement 'k' gires a l'esquerra Patricio2                                 #" << endl;
        cout << "# - prement 'l' gires a la dreta Patricio2                                   #" << endl;
        ...
        cout << "# - prement '3' activar/desactivar llum vermell per defecte desactivat.      #" << endl;
        cout << "# - prement 'j' camara del ninot de neu.                                     #" << endl;
        cout << "##############################################################################" << endl;
    }
    else if  (caracter == 27) {
        glutLeaveMainLoop();
